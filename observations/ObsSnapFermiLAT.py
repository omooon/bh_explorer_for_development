...
# this class based on galactic coordinate
class ObsSnapFermiLAT:
    def __init__(
        ...
        bkg_models : `~ gammapy.modeling.models.Model`
            
        map_geom : `~ gammapy.maps.WcsGeom` or `~ gammapy.maps.HpxGeom`

        count_radius : string

        """
        # 期待されるデータセットかのチェックk
        # onもoffもどっちもこれ一つでアクセス可能
        self._map_dataset = obs_map_dataset
    ...
    def create_from_fermitools(
        ...
        #ft1,ft2の親ディレがweeklyかserverかで、分岐して作る
        from pbh_explorer.fermi_lat.weekly import WeeklyDataAnalysis
        weekly.run_fermi_pipeline(ft1_filepath, ft2_filepath)
        obs_map_dataset = weekly.create_map_dataset()
        return cls(obs_map_dataset)
        
    @classmethod
    def create_from_gammapy(cls):
        pass
        
    @property
    def map_dataset(self):
        return self._obs_map_dataset
